...
import { ragDnaTable, tasksTable } from '@workspace/db-v2';
import { eq, and, inArray } from 'drizzle-orm';
import { validateEntry, runDiscoverySweep } from '../utils/vaelEngine.js';
import type { DiscoveryProposal } from '../utils/vaelEngine.js';
...
async function recordVaelRun(taskType: 'full_sweep' | 'validation', summary: string, durationSec: number): Promise<void> {
  try {
    const rows = await db.select().from(tasksTable).where(
      eq(tasksTable.operatorId, VAEL_OPERATOR_ID)
    );
    const task = rows.find(r => (r.payload as Record<string, unknown>)?.vaelTaskType === taskType);
    if (!task) return;
    const currentPayload = (task.payload as Record<string, unknown>) ?? {};
...
const FULL_BUDGET_MS = 270_000;
const FAST_BUDGET_MS = 55_000;
...
async function classifyScope(content: string): Promise<{
  ...
  try {
    ...
    const parsed = JSON.parse(res.content.trim()) as {
      dna_scope?: string;
...
export async function runVaelFullSweep(): Promise<void> {
  ...
  try {
    ...
    const discovery = timer.hasTime(60_000) ? await runDiscoveryPhase(timer) : { proposed: 0, seeded: 0, flagged: 0 };

    const summary =
      `validated=${validation.validated} approved=${validation.approved} revised=${validation.revised} rejected=${validation.rejected} | ` +
      `discovered=${discovery.proposed} seeded=${discovery.seeded} flagged=${discovery.flagged}`;
...
export async function runVaelValidationOnly(): Promise<void> {
  if (state.isRunning) {
    console.log('[VAEL] Already running — skipping validation cycle');
    return;
  }
  state.isRunning = true;
...
export async function runVaelCron(mode: 'full_sweep' | 'validation'): Promise<void> {
  if (mode === 'full_sweep') {
    return runVaelFullSweep();
  }
  return runVaelValidationOnly();
}
