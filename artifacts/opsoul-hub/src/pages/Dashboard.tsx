...
function HealthBadge({ operatorId }: { operatorId: string }) {
  ...
  return (
    <Badge variant="outline" className={`${colorClass} font-label text-[10px] px-2 py-0.5`}>
      {score}% · {label}
    </Badge>
  );
}
...
function OperatorCard({ operator, onClick }: { operator: Operator; onClick: () => void }) {
  ...
  return (
    <div
      className="group relative overflow-hidden rounded-2xl border border-border/30 bg-white hover:border-primary/40 transition-colors cursor-pointer flex flex-col"
      onClick={onClick}
        ...
        {operator.roles && operator.roles.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {operator.roles.map((role) => (
              <span key={role} className="font-label text-[10px] text-muted-foreground/70 bg-muted/40 border border-border/30 rounded px-1.5 py-0.5 leading-none">
                {role}
              </span>
            ))}
          </div>
    ...
    <div className="space-y-6 animate-in fade-in duration-300 max-w-xl">
      ...
      <div className="bg-white rounded-2xl border border-border/30 p-6 space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="text-sm bg-background/60 border border-border/30 rounded-lg px-3 py-2.5 text-muted-foreground select-all">
            {owner?.email}
          </div>
        ...
        {/* Display name */}
        <div className="space-y-1.5">
          <label className="text-xs text-muted-foreground tracking-wider">Display Name</label>
          <input
            type="text"
            value={nameInput}
            onChange={e => setNameInput(e.target.value)}
            placeholder="Your name"
            className="w-full text-sm bg-background/60 border border-border/30 rounded-lg px-3 py-2.5 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/50 transition-colors"
          />
        </div>
      ...
      <div className="bg-white rounded-2xl border border-border/30 p-6 space-y-5">
        ...
        <div className="space-y-1.5">
          <label className="text-xs text-muted-foreground tracking-wider">Current Password</label>
          <div className="relative">
            <input
              ...
              placeholder="••••••••••••"
              className="w-full text-sm bg-background/60 border border-border/30 rounded-lg px-3 py-2.5 pr-10 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/50 transition-colors"
            />
            <button
              type="button"
        ...
        <div className="space-y-1.5">
          <label className="text-xs text-muted-foreground tracking-wider">New Password</label>
          <div className="relative">
            <input
          ...
          <p className="text-[10px] text-muted-foreground/60">Minimum 12 characters. All sessions will be invalidated.</p>
        </div>
...
function StatCard({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <div className="bg-white rounded-xl border border-border/30 p-5 relative overflow-hidden">
      ...
      <div className="font-label text-[10px] tracking-[0.2em] text-muted-foreground">{label}</div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white rounded-2xl border border-border/30 p-6 space-y-4">
        <h3 className="font-label text-[10px] tracking-[0.2em] text-muted-foreground">Operator Roster</h3>
        {count === 0 ? (
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="status-beacon" />
              <span className="font-label text-[10px] tracking-[0.2em] text-secondary">Active</span>
            </div>
            <h3 className="font-headline text-2xl font-bold text-primary">Starter</h3>
            <p className="text-sm text-muted-foreground mt-0.5">Free plan · No credit card required</p>
          </div>
          <div className="text-right">
            <div className="font-headline text-3xl font-bold text-foreground">$0</div>
            <div className="font-label text-[10px] text-muted-foreground">/ month</div>
          </div>
        </div>
    ...
    <div className="space-y-6 animate-in fade-in duration-300 max-w-xl">
      ...
      <div className="bg-white rounded-2xl border border-destructive/20 p-6 space-y-4 relative overflow-hidden">
        ...
        <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-4 space-y-3">
          ...
          <button
            disabled
            className="text-xs px-4 py-2 rounded-lg border border-destructive/30 text-destructive/50 bg-destructive/5 cursor-not-allowed opacity-60"
          >
        ...
        {owner?.isSovereignAdmin && (
          <Link href="/admin">
            <button className="flex items-center gap-2.5 pl-3 pr-3 py-2 rounded-md text-sm font-label w-full text-left transition-all border border-primary/20 bg-accent hover:bg-primary/10 hover:border-primary/40 hover:text-primary text-primary/70 group">
              ...
              <span className="shrink-0 text-[9px] px-1.5 py-0.5 rounded bg-primary/15 border border-primary/30 text-primary tracking-wider">
                SA
              </span>
            </button>
          </Link>
