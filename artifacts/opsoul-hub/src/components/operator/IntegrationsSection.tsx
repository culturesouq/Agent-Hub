...
/**
 * Tools each integration unlocks when connected — kept in sync with the
 * registry in artifacts/opsoul-api/src/utils/toolRegistry.ts (wave 2 +
 * wave 3). Surfacing this on the card so the owner knows "Gmail connected
 * → 3 tools" without digging through the Skills tab.
 */
const TOOLS_PER_INTEGRATION: Record<string, string[]> = {
  telegram:        ["send_telegram"],
  whatsapp:        ["send_whatsapp"],
  slack:           ["send_slack", "slack_search"],
  gmail:           ["gmail_send", "gmail_search", "gmail_read"],
  google_calendar: ["calendar_create_event", "calendar_list_events"],
  google_drive:    ["drive_search", "drive_read_file"],
  github:          ["github_create_issue", "github_search", "github_read_file"],
  notion:          ["notion_search", "notion_create_page"],
  hubspot:         ["hubspot_search_contact", "hubspot_create_deal"],
  linear:          ["linear_create_issue", "linear_search"],
  discord:         [], // Discord connector tile only; tools land in a follow-up
};

// Exported so the Google Workspace section-header can render the brand mark
// without re-declaring the SVG. Retained per [[expand-never-cut]] — the
// per-product logos (Gmail / Calendar / Drive) handle the tiles; this is
// the parent-group glyph.
export function GoogleLogo() { return <svg width="20" height="20" viewBox="0 0 18 18"><path d="M17.64 9.205c0-.639-.057-1.252-.164-1.841H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z" fill="#4285F4"/><path d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z" fill="#34A853"/><path d="M3.964 10.71A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z" fill="#FBBC05"/><path d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.958L3.964 6.29C4.672 4.163 6.656 3.58 9 3.58z" fill="#EA4335"/></svg>; }
...
function OutboundStat({ outbound }: { outbound: Integration["outbound"] }) {
  if (!outbound) return null;
  return (
    <div className="font-mono text-[10px] text-muted-foreground/70 pl-5">
      {outbound.count > 0
        ? <>📤 {outbound.count} message{outbound.count === 1 ? "" : "s"} sent{outbound.lastSentAt ? <> · last {new Date(outbound.lastSentAt).toLocaleString()}</> : null}</>
        : <>No outbound messages sent yet</>}
    </div>
  );
}

function ToolsUnlocked({ connectorId }: { connectorId: string }) {
  const [open, setOpen] = useState(false);
  const tools = TOOLS_PER_INTEGRATION[connectorId] ?? [];
  ...
  return (
    <div className="rounded-md border border-primary/20 bg-primary/5 px-2.5 py-1.5">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between gap-2 font-mono text-[10px] text-primary hover:underline"
      >
        <span>Unlocks {tools.length} tool{tools.length === 1 ? "" : "s"} for the operator</span>
        <ChevronDown className={`w-3 h-3 transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
      {open && (
        <ul className="mt-1 pt-1 border-t border-primary/15 space-y-0.5">
          {tools.map(t => (
            <li key={t} className="font-mono text-[10px] text-muted-foreground">• {t}</li>
          ))}
        </ul>
      )}
    </div>
    ...
    <CardShell brand={connector.brand} logo={connector.logo} name={connector.name} description={connector.description}>
      {connected ? (
        <div className="space-y-2">
          ...
          {connector.id === "slack" && <OutboundStat outbound={connected.outbound} />}
          <ToolsUnlocked connectorId={connector.id} />
        </div>
      ) : !showForm ? (
    ...
    <CardShell brand={connector.brand} logo={connector.logo} name={connector.name} description={connector.description}>
      {connected ? (
        <div className="space-y-2">
          ...
          <OutboundStat outbound={connected.outbound} />
          <ToolsUnlocked connectorId={connector.id} />
        </div>
      ) : !showForm ? (
        <div className="flex items-center justify-between pt-2 border-t border-border/20 mt-auto">
