...
//
// Archetype + role lists live in `../constants/archetypes.ts` (single source
// of truth across this file + routes/operators.ts). Aliases BIRTH_ARCHETYPES
// + BIRTH_ROLES are imported at the top of the file under those local names
// for grep-friendliness in the birth-extraction prompt code below.
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  // System prompt = identity only (Layer 0 + Layer 1). No KB, no memory.
  // KB is retrievable via kb_search tool when the operator needs it.
  // Memory is retrievable via memory tools on demand.
  // Injecting KB/memory as noise into the system prompt is the root cause
